...
def filter_data(data: pd.DataFrame) -> pd.DataFrame:
    return data[
        (
            data[DataNewColumns.LOADED_DAY.value]
            >= datetime.date(year=2021, month=12, day=3)
        )
        & (
            data[DataNewColumns.FIRST_DAY_OF_WEEK.value]
            >= datetime.date(year=2021, month=9, day=1)
        )
    ]
...
def prep_data(data: List[Dict]) -> pd.DataFrame:
    ...
    return prepped_data.sort_values(
        by=[DataNewColumns.LOADED_DAY.value, DataNewColumns.FIRST_DAY_OF_WEEK.value]
    )
