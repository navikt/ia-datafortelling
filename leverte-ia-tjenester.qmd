...
# Unike virksomheter
```{python}
from leverte_ia_tjenester_plots import unike_virksomheter_bulletlist
unike_virksomheter_bulletlist(data)
```

```{python}
from leverte_ia_tjenester_plots import plot_unike_bedrifter
plot_unike_bedrifter(data)
```

## Kululativt historgram av unike virksomheter
```{python}
from leverte_ia_tjenester_plots import plot_antall_unike_virksomheter_kumulativt
...
```{python}
from leverte_ia_tjenester_plots import plot_tjenester_per_applikasjon
plot_tjenester_per_applikasjon(data)
```

### Siste 24 måneder
```{python}
from leverte_ia_tjenester_plots import tabell_antall_per_app_siste_24_mnd
tabell_antall_per_app_siste_24_mnd(data)
```

### Siste 30 dager
```{python}
from leverte_ia_tjenester_plots import  tabell_antall_per_app_siste_30_dager
tabell_antall_per_app_siste_30_dager(data)
```

# Informasjons- vs. interaksjonstjeneste i forebyggingsplan
```{python}
from leverte_ia_tjenester_plots import plot_informasjon_vs_interaksjon
plot_informasjon_vs_interaksjon(data)
```

# Fordeling antall ansatte
```{python}
from leverte_ia_tjenester_plots import plot_fordeling_antall_ansatte
...
```{python}
from leverte_ia_tjenester_plots import plot_tilbakevendende_brukere
