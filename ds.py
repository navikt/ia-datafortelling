...
def create_bar_plot(data: pd.DataFrame) -> str:
    max_value = (data[[DataNewColumns.LOADED_DAY_STR.value,
                       DataNewColumns.FORMATTED_WEEK.value,
                       DataInColumns.COUNT.value]]
                 .groupby(by=[DataNewColumns.LOADED_DAY_STR.value,
                              DataNewColumns.FORMATTED_WEEK.value])
                 .sum()
                 .reset_index(drop=False))[DataInColumns.COUNT.value].max()
    ...
    fig = px.bar(data,
                 ...
                 custom_data=[DataNewColumns.LOADED_DAY_STR.value,
                              DataInColumns.STATUS.value],
                 width=1300,
                 height=600)
...
def create_datastory_pam(data: pd.DataFrame) -> DataStory:
    ...
    ds.markdown(md="""Denne datafortellingen illustrerer hvor mange brukernotifikasjoner arbeidsplassen.no har sendt ut og 
    ...
    ds.markdown(md="""Denne grafen viser hvor mange nyregistrerte i NAV som har fått beskjed om å legge inn eller 
    ...
    ds.plotly(fig=fig)
    ds.markdown(md="""Ved å bevege «slideren» i bunn av grafen, kan du bestemme fra hvilket tidspunkt du ønsker å 
    se status for.""")
